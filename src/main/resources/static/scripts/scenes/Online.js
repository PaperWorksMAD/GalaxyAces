...
export class Online extends Phaser.Scene {
	...
	create() {
		connection = new WebSocket('ws://localhost:8080/galaxy');
			...
			if (pillado == false) {
				...
				if (playername !== '') {
					...
					if (permitido) {
						...
						this.shipIndex2 = 2;
						createPlayer();
						var msg = {name: "mensaje", message: "hola", x: 0, y: 0}
			...
			if (numjugadoreslistos == 2) {
				...
				this.time.addEvent({
					delay: 2000, callback: function() {
						this.reiniciar();
						if (naveaux == 1) {
							this.scene.start(sceneManager.SCENES.ONLINEGAME, { shipIndex1: this.shipIndex1, shipIndex2: this.shipIndex2, efSound: this.efecsound, efvol: this.efvol, idAux: this.idAux, conexion : connection });
						}
						if (naveaux == 2) {
							this.scene.start(sceneManager.SCENES.ONLINEGAME2, { shipIndex1: this.shipIndex1, shipIndex2: this.shipIndex2, efSound: this.efecsound, efvol: this.efvol, idAux: this.idAux, conexion: connection });
						}
					}, callbackScope: this, loop: false
				});
